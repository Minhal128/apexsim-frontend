...
import { APP_LANGUAGE_EVENT, APP_LANGUAGE_STORAGE_KEY, AppLanguageCode, getAppLanguage, t } from '@/lib/i18n';
...
function TradingPageContent() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchMarketData, 5000); // 5s fallback polling
  ...
  return (
    <div className="bg-[#181818] px-2 h-full text-gray-300 font-manrope">
      ...
      <div className="flex flex-col md:flex-row h-full gap-px bg-white/5">
        ...
        <OrderBook symbol={assetSymbol} currentPrice={(marketInfo?.price ?? marketInfo?.value ?? marketInfo?.usd ?? marketInfo?.regularMarketPrice ?? selectedCoin.current_price)} />
        <TradeForm symbol={assetSymbol} currentPrice={(marketInfo?.price ?? marketInfo?.value ?? marketInfo?.usd ?? marketInfo?.regularMarketPrice ?? selectedCoin.current_price)} />
      </div>
